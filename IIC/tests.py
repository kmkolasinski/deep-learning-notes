...
train_dataset, dataset_info = tfds.load(name="fashion_mnist",
                                        split=tfds.Split.TRAIN,
                                        as_supervised=True, with_info=True)
test_dataset = tfds.load(name="fashion_mnist", split=tfds.Split.TEST,
                         as_supervised=True)
...
train_dataset = train_dataset.shuffle(1000).repeat(-1).batch(batch_size).map(
    normalize_labels)
...
base_model = ResNet(model_params=params, input_shape=input_shape, weights=None,
...
cls_model.compile(optimizer=optimizer, loss='sparse_categorical_crossentropy',
                  metrics=['accuracy'])
...
cls_model.fit(
    train_dataset, verbose=1, epochs=10,
...
def strong_aug(p=0.5):
    return Compose([
        ...
        #         ], p=0.2),
        ShiftScaleRotate(shift_limit=0.2, scale_limit=0.2, rotate_limit=10,
                         p=1.0),
        #         OneOf([
...
train_dataset = train_dataset.shuffle(1000).repeat(-1).map(
    normalize_labels_and_augment).batch(batch_size).prefetch(10)
test_dataset = test_dataset.batch(batch_size).map(normalize_labels)
...
cls_model.fit(
    train_dataset, verbose=1, epochs=5,
    validation_data=test_dataset,
    validation_steps=num_eval_steps,
    steps_per_epoch=num_train_steps
)
...
def strong_aug(p=0.5):
    return Compose([
        ...
        #             Blur(blur_limit=3, p=0.1),
        #         ], p=0.2),
        ShiftScaleRotate(shift_limit=0.2, scale_limit=0.1, rotate_limit=45,
                         p=1.0, border_mode=1),
        #         OneOf([
        #             OpticalDistortion(p=0.3),
        #             GridDistortion(p=0.1),
...
iic_train_dataset = train_dataset.shuffle(1000).repeat(-1).map(
    normalize_labels_and_augment_v2).batch(batch_size).prefetch(10)
iic_test_dataset = test_dataset.map(normalize_labels_and_augment_v2).batch(
    batch_size).prefetch(10)
...
cls_model = keras.models.Model(
    ...
    outputs={"image_output": image_output, "tf_image_output": tf_image_output,
             'distance': distance}
)
...
cls_model.fit(
    iic_train_dataset, verbose=1, epochs=1,
    #     validation_data=test_dataset,
    #     validation_steps=num_eval_steps,
    steps_per_epoch=num_train_steps
)
